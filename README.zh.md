...
- [Bun](https://bun.sh) ≥ 1.3 —— 同时作为 JS 运行时和测试 runner，整个项目
  不依赖任何 Node 工具链。
...
两层，全部跑在 Bun 内置的 test runner 上（零 dev dependencies），都已在干净
checkout 上验证通过：

```sh
bun test test/unit                   # 33 个单测，~1.7 秒，无需任何凭证
MAPTILER_KEY=... bun test test/e2e   # 完整管线：渲染 demo GPX → MP4 → scene_score 回归，~3:20
```

或者走 package.json 里的 script：
...
单测在 `test/unit/` 下，覆盖 `src/render-config.js` 的时长计算、
`src/parse-gpx.js` 直接吃仓库里那份 `activity_580930440.gpx`、
...
独立目录里，`bun test test/unit` 扫不到；
**没设 `MAPTILER_KEY` 时自动 skip**（通过 `test.skipIf`），CI / 冷克隆
不会因此挂掉。

GitHub Actions（`.github/workflows/test.yml`）在每次 push / PR 自动跑单测。
e2e 那层挂在手动 `workflow_dispatch` 下（要勾 `run_e2e`），只有明确触发才会
消耗 `MAPTILER_KEY` secret。
...
`bun run test:e2e` 就是把这个检查包在一次完整渲染外面的端到端版本 —— 想一键
...
scripts/
  regress.js          scene_score 回归检查（被 bun run regress 调）

test/
  unit/                 快速单测（bun test test/unit）
